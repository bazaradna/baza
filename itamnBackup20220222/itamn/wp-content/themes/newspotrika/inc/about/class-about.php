...
if ( ! class_exists( 'newspotrika_About' ) ) {

	class newspotrika_About {
		private $config;
		private $theme_name;
		private $logo_url;
		private $logo_link;
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof newspotrika_About ) ) {
				self::$instance = new newspotrika_About;
		...
		public function setup_config() {
			...
			$this->logo_url      = isset( $this->config['logo_url'] ) ? $this->config['logo_url'] : get_template_directory_uri() . '/inc/about/images/logo.png';
			$this->logo_link     = isset( $this->config['logo_link'] ) ? $this->config['logo_link'] : 'https://codecorns.com/';
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$dismiss_url = wp_nonce_url( add_query_arg( array( 'newspotrika-np-dismiss' => '1' ), admin_url( 'themes.php' ) ), 'newspotrika-action-dismiss-' . get_current_user_id(), 'newspotrika-dismiss-nonce' );
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'newspotrika' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'newspotrika' ), $this->theme_name ) . '</a>' ) . '<a href="' . esc_url( $dismiss_url ) . '" class="btn-np-dismiss">' . esc_html__( 'Dismiss', 'newspotrika' ) . '</a></p>';
		}
		...
		public function setup_actions() {
			...
			add_action( 'wp_ajax_np_about_action_dismiss_recommended_action', array( $this, 'dismiss_recommended_action_callback' ) );
			add_action( 'wp_ajax_nopriv_np_about_action_dismiss_recommended_action', array( $this, 'dismiss_recommended_action_callback' ) );
		}
		...
		function hide_notice() {
			// Verify nonce.
			if (! ( isset( $_GET['newspotrika-dismiss-nonce'] ) && wp_verify_nonce( sanitize_key( $_GET['newspotrika-dismiss-nonce'] ), 'newspotrika-action-dismiss-' . get_current_user_id() ) ) ) {
				return;
			}

			if ( 1 === absint( $_GET['newspotrika-np-dismiss'] ) ) {
				update_user_meta( get_current_user_id(), 'newspotrika_np_dismissed', 1 );
			}
		}
		...
		public function render_about_page() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<div class="wrap about-wrap np-wrap">';
		...
		public function activation_admin_notice() {
			...
			$dismissed_status = get_user_meta( get_current_user_id(), 'newspotrika_np_dismissed', true );
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'newspotrika-about', get_template_directory_uri() . '/inc/about/css/about' . $min . '.css', array(), '1.0.1' );
				wp_enqueue_script( 'newspotrika-about', get_template_directory_uri() . '/inc/about/js/about' . $min . '.js', array( 'jquery' ), '1.0.1' );
				$js_vars = array(
					'ajaxurl' => esc_url( admin_url( 'admin-ajax.php' ) ),
				);
				wp_localize_script( 'newspotrika-about', 'startMagazineAboutObject', $js_vars );
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'newspotrika' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'newspotrika' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'newspotrika' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'newspotrika' );
		...
		public function useful_plugins() {
			...
			if ( isset( $tgmpa ) ) {
				...
				if ( true === $complete ) {
					echo '<div class="all-active"><span class="dashicons dashicons-yes"></span>&nbsp;' . esc_html__( 'Great, all recommended plugins are intalled and active.', 'newspotrika' ) . '</div>';
				}
				...
				if ( true !== $complete ) {
					echo '<div class="button-wrap"><a href="' . esc_url( $url ) . '" class="button button-primary">' . esc_html__( 'Manage Plugins', 'newspotrika' ) . '</a></div>';
				}
